...
这里主要是指qianfanmall-wx-api、qianfanmall-wx和renard-wx三个模块的相关问题。
...
2. 开发者在qianfanmall-wx、renard-wx和qianfanmall-core模块的appid信息设置成自己申请的信息。
...
微信开发者工具中修改appid以后，请关闭qianfanmall-wx项目或者微信开发者工具，重新启动导入qianfanmall-wx。
...
```
qianfanmall.wx.app-id=
qianfanmall.wx.app-secret=
qianfanmall.wx.mch-id=
qianfanmall.wx.mch-key=
qianfanmall.wx.notify-url=
```
...
这里主要是指qianfanmall-admin-api和qianfanmall-admin两个模块的相关问题。
...
2. 其次，需要明白报错的地方，是qianfanmall-admin/src/utils/request.js文件中;
...
qianfanmall-db模块的application-db.yaml资源文件中reasonable是true
...
这里主要是指qianfanmall-db、qianfanmall-core和qianfanmall-all模块三个模块的相关问题。
...
```
org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): org.linlinjava.qianfanmall.db.dao.XXXX
```
...
The error may involve org.linlinjava.qianfanmall.db.dao.LitemallSystemMapper.selectByExample-Inline
...
use qianfanmall;
create user 'qianfanmall'@'localhost' identified by 'qianfanmall123456';
grant all privileges on qianfanmall.* to 'qianfanmall'@'localhost';
flush privileges;
```
...
应该是qianfanmall-admin模块和qianfanmall-vue模块的node_modules文件夹导致的。
node_modules是qianfanmall-admin和qianfanmall-vue模块所依赖的项目库，可能有近200M的文件。
而IDEA如果没有设置，则可能尝试对该文件夹进行解析索引，从而导致卡断。

解决方案：

1. 先关闭IDEA，然后删除qianfanmall-admin和qianfanmall-vue模块内的node_modules文件夹；
2. 然后分别创建空的node_modules文件夹；
3. 重新打开IDEA，分别设置qianfanmall-admin模块和qianfanmall-vue模块的node_modules文件夹Excluded状态。
