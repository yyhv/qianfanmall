...
* 删除了一些目前不必要的表；
* 删除了表中一些目前不必要的字段；
* 行政区域数据def_region没有采用原nideshop中的数据，而是采用了[Administrative-divisions-of-China](https://github.com/modood/Administrative-divisions-of-China)；
* 表中的某些字段采用JSON存储；
...
综上，这里qianfanmall真正必须运行的sql文件是def_table.sql，其他两个sql文件开发者自行决定如何是否使用。
...
这里商品存在商品表（def_goods)，商品属性表（def_goods_attribute），商品规格表（def_goods_specification），商品货品表（def_goods_product）四种表
...
def_region表保存了行政区域信息，包括省级、市级、县级三个等级，
...
由def_order表和def_order_goods表保存。
...
def_order表中存在`aftersale_status`字段，记录订单售后状态。
而具体的售后记录则是def_aftersale表记录。
...
这里需要补充的是：订单def_order表的`aftersale_status`字段，和订单售后def_aftersale
...
在def_order表中存在`comments`字段，表示有几个订单商品没有评价；
而在def_order_goods表中存在`comment`字段，表示当前订单商品的评论ID。
...
评论表def_comment保存评论相关的信息，其中最关键的是`type`字段和`value_id`字段。
...
团购是由团购规则表def_groupon_rules和团购活动表def_groupon组成。

管理员在管理后台对一些商品配置团购规则，保存在def_groupon_rules表中。

用户在小商场中则看到团购规则给出的优惠信息。
接下来用户存在两种操作：
第一种是，用户开团，保存在def_groupon中，用户主动分享商品团购页面给朋友；
第二张是，用户参团，也保存在def_groupon中。

只有开团人数符合团购规则条件，创建的订单才会有效，否则管理员需要退款取消当前团购。

### 2.7 优惠券设计

优惠券由def_coupon表和def_coupon_user表组成：
* def_coupon表，是优惠券基本信息及使用规则。
* def_coupon_user表，是用户优惠券领取和使用的记录。
...
这里需要指出的是，def_coupon表和def_coupon_user表都有status字段。

def_coupon表的status字段，包含以后三种状态：
* 正常可用，
* 已过期，
* 已下架，

def_coupon_user表的status字段，包含以后三种状态：
* 未使用，
* 已使用，
* 已过期，
* 已下架，
...
除了以上表，数据库还存在其他一些业务表，例如专题表def_topic，
但是都很直观，不需要多讨论。
