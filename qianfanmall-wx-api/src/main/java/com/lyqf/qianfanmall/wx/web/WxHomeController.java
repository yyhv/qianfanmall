...
import com.lyqf.qianfanmall.wx.service.WxGrouponRuleService;
...
public class WxHomeController {
    ...
    @Autowired
    private QianfanmallAdService adService;
...
//        }
    ...
    private List<Map> getCategoryList() {
        ...
        List<QianfanmallCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (QianfanmallCategory catL1 : catL1List) {
            List<QianfanmallCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (QianfanmallCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<QianfanmallGoods> categoryGoods;
            if (l2List.size() == 0) {
